...
const API_URL = 'https://campus-navigation-ljmh.onrender.com/api/rooms';
let roomsData = {};
...
async function loadRooms() {
    ...
    if (!response.ok) {
        throw new Error('Could not load rooms from backend');
    }
    ...
    rooms.forEach(room => {
        roomsData[room.id.toUpperCase()] = room;
    });
}
...
document.addEventListener('DOMContentLoaded', async function() 
{
    ...
    try {
        ...
        console.error(error);
        alert('Backend is not running. Please start the backend, then refresh this page.');
    }
});
...
function createQuickAccess() 
{
    ...
    const quickRooms = ['CR101', 'CR205', 'EXAM', 'LAB1', 'LIB'];
